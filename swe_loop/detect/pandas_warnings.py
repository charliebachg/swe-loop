...
CLASSES = [
    ...
    ("datetime-unit",      r"unit=|datetime64\[ns\]|resolution|to_datetime.*infer|infer_datetime_format"),
    ("offset-alias",       r"'[A-Z]+' is deprecated|offset alias|freq=.*deprecated"),
    ("concat-empty",       r"concat.*empty|empty entries|all-NA"),
    ("applymap",           r"applymap"),
    ("is_categorical",     r"is_categorical_dtype|is_datetime64tz_dtype|is_sparse|is_interval_dtype|is_period_dtype"),
...
PANDAS_HINT = re.compile(r"pandas|DataFrame|Series|dtype|fillna|inplace|groupby|to_datetime|concat", re.I)
...
def parse(path):
    ...
    for tc in root.iter("testcase"):
        ...
        for tag in ("failure", "error", "skipped"):
            el = tc.find(tag)
            if el is not None:
                outcome = "failed" if tag == "failure" else ("error" if tag == "error" else "skipped")
                msg = el.get("message") or ""
                text = el.text or ""
                break
        out[tid] = dict(outcome=outcome, message=msg, text=text, testfile=tc.get("file") or "")
    return out

import os
ROOT = os.environ.get("REPO_ROOT", "").rstrip("/") + "/" if os.environ.get("REPO_ROOT") else ""
FRAME = re.compile(r"^(\S+?\.py):(\d+): ?(\S*)\s*$", re.M)
def frames(text):
    return [(m.group(1).replace(ROOT, ""), int(m.group(2)), m.group(3)) for m in FRAME.finditer(text or "")]
def locate(text, msg):
    ...
    if sup:   site, where = sup[-1], "superset"
    elif tst: site, where = tst[-1], "test-only"
    else:     site, where = ("", 0, ""), "unknown"
...
def main():
    r1 = parse(S / "r1-p2-default.xml"); r2 = parse(S / "r2-detail.xml") or parse(S / "r2-p2-werror.xml"); r3 = parse(S / "r3-p3-default.xml")
    bad = {"failed", "error"}
    ...
    for tid, r in r2.items():
        if r["outcome"] in bad and tid not in base:
            ...
            rows.append(dict(source="r2-forward", test=tid, file=f, line=ln, func=fn, where=where, testfile=tf,
    ...
    for tid, r in r3.items():
        if r["outcome"] in bad and tid not in base:
            f, ln, fn, where, tf = locate(r["text"], r["message"])
            rows.append(dict(source="r3-break", test=tid, file=f, line=ln, func=fn, where=where, testfile=tf,
                             cls=classify(r["message"], r["text"]),
                             pandas=bool(PANDAS_HINT.search(r["message"] + r["text"])),
                             message=(r["message"] or "").strip()[:400]))
    ...
    lines = ["# pandas 3 migration inventory, unit tests", "",
             f"| run | tests | failed/error |", "|---|---|---|",
    ...
    for src in ("r2-forward", "r3-break"):
        ...
        lines += [f"## {src}: by class", "", "| class | rows | distinct files | distinct tests |", "|---|---|---|---|"]
        byc = defaultdict(list)
        for r in sub: byc[r["cls"]].append(r)
        for c, rs in sorted(byc.items(), key=lambda kv: -len(kv[1])):
            lines.append(f"| {c} | {len(rs)} | {len({r['file'] for r in rs if r['file']})} | {len({r['test'] for r in rs})} |")
        ...
        for r in sub:
            ...
            seen.add(key); lines.append(f"- [{r['cls']}] `{r['file']}:{r['line']}` {r['message'][:160]}")
